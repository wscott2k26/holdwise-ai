...
export const speedEngine={id:'speed',createGame,legalActions,applyAction,isTerminal:state=>Boolean(state.gameComplete)||state.players.some(player=>player.hand.length===0&&player.stock.length===0)||permanentStall(state)||permanentStall(state)||permanentStall(state),result,coachFacts};
